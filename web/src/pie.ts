...
export function actualizarPie(): void {
  ...
  if (m.hitos.includes('final')) partes.push('descenso completo');
  if (descensoAbierto() && !partes.length) partes.push('expediente habilitado por clave');

  texto.textContent = partes.length
    ? `Consta: ${partes.join(' · ')}.`
    : 'No consta ningún descenso. Lo que baje en el juego se anotará aquí.';
